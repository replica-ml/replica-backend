...
async fn test_upsert_profile() {
    ...
    let profile: Profiles = serde_json::from_slice(&bytes).unwrap();
    let expect = Profiles {
        alias,
...
async fn test_read_profile() {
    ...
    let alias = format!("{}-alias", USERNAME);
    let req = test_profile_api::post(&token, &alias, USERNAME);
    let upserted_profile: Profiles = actix_web::test::call_and_read_body_json(&app, req).await;

    let req = test_profile_api::get(&token);
    let read_profile: Profiles = actix_web::test::call_and_read_body_json(&app, req).await;
    let expect = Profiles {
        alias,
        username: String::from(USERNAME),
